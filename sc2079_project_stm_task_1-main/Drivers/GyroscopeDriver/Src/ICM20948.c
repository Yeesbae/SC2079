...
uint8_t const selectAccelSensitivity = ACCEL_FULL_SCALE_2G;
uint8_t const selectGyroSensitivity = GYRO_FULL_SCALE_250DPS;
uint8_t const selectI2cAddress = 0;
...
int ICM20948_init(IMU_Data *dev, I2C_HandleTypeDef *hi2c) {
	dev->i2cHandle = hi2c;

	// status 1 = error, status 0 = success
	HAL_StatusTypeDef status = HAL_OK;
...
int ICM20948_readGyroscope_allAxises(IMU_Data* imu) {
    float* g = imu->gyro;                 // will be in deg/s
    I2C_HandleTypeDef *hi2c = imu->i2cHandle;

    HAL_StatusTypeDef st;
    uint8_t rd[6];

    st = _ICM20948_SelectUserBank(hi2c, selectI2cAddress, USER_BANK_0);
    if (st != HAL_OK) return 1;

    st = _ICM20948_BrustRead(hi2c, selectI2cAddress,
                             ICM20948__USER_BANK_0__GYRO_XOUT_H__REGISTER, 6, rd);
    if (st != HAL_OK) return 1;

    // SIGN-PRESERVE
    int16_t rx = (int16_t)((uint16_t)rd[0] << 8 | rd[1]);
    int16_t ry = (int16_t)((uint16_t)rd[2] << 8 | rd[3]);
    int16_t rz = (int16_t)((uint16_t)rd[4] << 8 | rd[5]);

    // Choose correct scale factor usage.
    // If your macros are LSB per (deg/s): use DIVIDE (131.072, 65.536, 32.768, 16.384)
    // If your macros are (deg/s) per LSB: use MULTIPLY (0.007629, 0.015259, 0.030518, 0.061035)
    float sf_lsb_per_dps;  // default assume LSB/(deg/s)
    switch (selectGyroSensitivity) {
        case GYRO_FULL_SCALE_250DPS:  sf_lsb_per_dps = 131.072f; break;
        case GYRO_FULL_SCALE_500DPS:  sf_lsb_per_dps = 65.536f;  break;
        case GYRO_FULL_SCALE_1000DPS: sf_lsb_per_dps = 32.768f;  break;
        case GYRO_FULL_SCALE_2000DPS: sf_lsb_per_dps = 16.384f;  break;
        default:                      sf_lsb_per_dps = 131.072f; break;
    }

    g[X] = (float)rx / sf_lsb_per_dps;   // deg/s
    g[Y] = (float)ry / sf_lsb_per_dps;   // deg/s
    g[Z] = (float)rz / sf_lsb_per_dps;   // deg/s
    return 0;
}
...
int ICM20948_readAccelerometer_allAxises(IMU_Data* imu) {
	HAL_StatusTypeDef status = HAL_OK;
	uint8_t readData[6];

	float* readings = imu->acc;
	I2C_HandleTypeDef * hi2c = imu->i2cHandle;
